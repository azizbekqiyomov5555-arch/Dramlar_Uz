...
5. ✅ PostgreSQL ga async fon rejimida saqlanadi (bot sekinlamaydi)
"""
...
DATABASE_URL = os.environ.get("DATABASE_URL") or ""   # Railway PostgreSQL URL
...
class RamCache:
    ...
    PostgreSQL faqat fon rejimida yoziladi — bot sekinlamaydi.
    """
    def __init__(self):
...
DB_STATUS = {
    ...
    "load_failed": False,    # PostgreSQL yuklanmadi (faqat log uchun, saqlashni bloklamaydi)
}
...
# ══════════════════════════════════════════════════════════
# POSTGRESQL OPERATSIYALARI (Railway)
# ══════════════════════════════════════════════════════════

def _get_pg_conn():
    """Railway PostgreSQL ga ulanish."""
    if not DATABASE_URL:
        raise RuntimeError("DATABASE_URL environment o'zgaruvchisi yo'q!")
    return psycopg2.connect(DATABASE_URL, connect_timeout=15)


def _init_postgres_table():
    """Jadval mavjud bo'lmasa — yaratadi. Ishga tushganda bir marta chaqiriladi."""
    if not DATABASE_URL:
        logger.warning("⚠️ DATABASE_URL yo'q — PostgreSQL ishlamaydi")
        return False
    try:
        conn = _get_pg_conn()
        ...
        cur.close()
        conn.close()
        logger.info("✅ PostgreSQL jadval tayyor (bot_data)")
        return True
    except Exception as e:
        logger.error(f"❌ PostgreSQL jadval yaratishda xato: {e}")
        return False


def _save_postgres(data: dict, retries: int = 3) -> bool:
    """Barcha ma'lumotlarni Railway PostgreSQL ga saqlaydi."""
    if not DATABASE_URL:
        return False

    movies = data.get("movies") if isinstance(data, dict) else None
    if isinstance(movies, dict) and len(movies) == 0 and len(RAM.movies) > 0:
        logger.warning("🛑 Bo'sh movies bilan Postgres ga yozish RAD ETILDI — himoya!")
        return False

    # Katta data ni qismlarga bo'lib saqlaymiz (movies alohida)
    records = {
        "movies":           data.get("movies", {}),
        "users":            data.get("users", {}),
        "channels":         data.get("channels", []),
        "simple_links":     data.get("simple_links", []),
    ...
    for attempt in range(retries):
        try:
            ...
            cur.close()
            conn.close()
            total_kb = len(json.dumps(records, ensure_ascii=False).encode()) / 1024
            logger.info(f"✅ PostgreSQL saqlandi ({total_kb:.1f} KB, {len(records)} kalit)")
            return True
        except Exception as e:
            logger.error(f"PostgreSQL saqlash #{attempt+1} xato: {e}")
...
def _load_postgres() -> dict | None:
    """Railway PostgreSQL dan yuklaydi."""
    if not DATABASE_URL:
        return None
    try:
        conn = _get_pg_conn()
        cur = conn.cursor()
        cur.execute("SELECT key, value FROM bot_data;")
        rows = cur.fetchall()
        cur.close()
        conn.close()

        if not rows:
            logger.info("ℹ️ PostgreSQL bo'sh — yangi baza")
            return {}

        data = {}
        for key, value in rows:
            data[key] = value  # psycopg2 JSONB ni dict ga aylantiradi

        movies_count = len(data.get("movies") or {})
        users_count  = len(data.get("users")  or {})
        logger.info(f"✅ PostgreSQL dan yuklandi: {movies_count} kino, {users_count} user")
        return data
    except Exception as e:
        logger.error(f"❌ PostgreSQL yuklash xato: {e}")
        return None
...
def _merge_db(blob: dict | None, local: dict | None) -> dict:
    """
    PostgreSQL va lokal fayldagi ma'lumotlarni BIRLASHTIRADI.
...
def db_initial_load():
    """
    Ishga tushganda:
    1. PostgreSQL jadval yaratiladi (agar yo'q bo'lsa)
    2. PostgreSQL DAN yuklaydi
    3. Lokal fayldan backup sifatida birlashtiradi
    4. RAMga yozadi
    5. Agar Postgres bo'sh bo'lsa — lokaldan tiklanadi
    """
    logger.info("🔄 Ma'lumotlar yuklanmoqda (Railway PostgreSQL)...")

    # Jadval yaratish
    _init_postgres_table()

    pg_data = _load_postgres()
    local   = _load_local()

    has_pg    = bool(pg_data  and (pg_data.get("movies")  or pg_data.get("users")))
    has_local = bool(local    and (local.get("movies")    or local.get("users")))

    if pg_data is None:
        logger.error("🛑 PostgreSQL dan yuklab bo'lmadi! Lokal fayldan davom etamiz.")
        DB_STATUS["storage_ok"] = False
        DB_STATUS["ram_only"]   = True
        DB_STATUS["last_err"]   = "PostgreSQL yuklanmadi — lokal fayldan yuklanmoqda"
    ...
    if not has_pg and not has_local:
        logger.warning("⚠️ Ma'lumot topilmadi — bo'sh RAM boshlanadi")
        RAM.loaded = True
        return

    merged = _merge_db(pg_data, local)
    RAM.from_dict(merged)
    EMOJI_IDS.clear()
    EMOJI_IDS.update(RAM.emoji_ids)

    pg_eps    = sum(len(m.get("episodes", []) or []) for m in (pg_data.get("movies")  or {}).values()) if has_pg    else 0
    ...
    logger.info(f"✅ Birlashtirildi → RAM: {len(RAM.movies)} kino, {merged_eps} qism, {len(RAM.users)} user "
                f"(pg: {len(pg_data.get('movies', {}) if has_pg else {})}/{pg_eps}, "
...
# ══════════════════════════════════════════════════════════
# SAQLASH — RAM → lokal + PostgreSQL (debounced background)
# ══════════════════════════════════════════════════════════

PG_DEBOUNCE = 12.0   # soniya — ketma-ket o'zgarishlardan keyin bir marta saqlash
...
def _ensure_lock():
    global _pg_save_lock
    if _pg_save_lock is None:
        _pg_save_lock = asyncio.Lock()
    return _pg_save_lock


async def _do_postgres_save() -> bool:
    """Haqiqiy PostgreSQL ga yozish (status ham yangilanadi)."""
    async with _ensure_lock():
        data = RAM.to_dict()
        ok = await asyncio.to_thread(_save_postgres, data)
...
async def _delayed_pg_save(delay: float):
    """Belgilangan vaqtdan keyin PostgreSQL ga yozish."""
    try:
        await asyncio.sleep(delay)
    except asyncio.CancelledError:
        return
    try:
        await _do_postgres_save()
    except Exception as e:
        logger.error(f"PostgreSQL debounced save xato: {e}")


async def schedule_save(delay: float = PG_DEBOUNCE):
    """
    RAMni saqlash navbatiga qo'yadi.
      • Lokal faylga DARHOL yoziladi
      • PostgreSQL ga `delay` soniyadan keyin yoziladi (debounce)
    """
    global _pg_timer_task
    ...
    if _pg_timer_task and not _pg_timer_task.done():
        _pg_timer_task.cancel()
    _pg_timer_task = asyncio.create_task(_delayed_pg_save(delay))


def save_sync():
    """Sinxron (thread) — lokal + PostgreSQL fon rejimida."""
    data = RAM.to_dict()
    _save_local(data)
    threading.Thread(target=_save_postgres, args=(copy.deepcopy(data),), daemon=True).start()


async def save_now() -> bool:
    """DARHOL saqlash — debounce taymerni bekor qiladi va PostgreSQL ga yozadi."""
    global _pg_timer_task
    if _pg_timer_task and not _pg_timer_task.done():
        _pg_timer_task.cancel()
    try:
        ...
        logger.error(f"Lokal saqlash xato: {e}")
    return await _do_postgres_save()


async def save_ram_only():
    """Faqat lokal faylga yozadi — PostgreSQL ga TEGMAYDI."""
    try:
        _save_local(RAM.to_dict())
    except Exception as e:
...
def _gsheet_log_user(user_id: int, name: str, username: str):
    pass  # GSheet o'chirildi — PostgreSQL ga saqlanadi
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "refresh_stats":
        ...
        if DB_STATUS["ram_only"]:
            storage_line = (f"\n\n🔴 <b>Storage: RAM ONLY</b>\n"
                           f"PostgreSQL ishlamayapti! Xato: <b>{DB_STATUS['fail_count']}</b>x")
    ...
    if data.startswith("finish_movie|"):
        ...
        await sm(context.bot, uid, "💾 Bazaga (PostgreSQL) saqlanmoqda, kuting...")
        ok = await save_now()
        if not ok:
        ...
        if ok:
            ...
            await sm(context.bot, uid,
                f"⚠️ Lokal saqlandi, lekin PostgreSQL xato berdi.\n"
    ...
    if _btn_match("stat"):
        ...
        if DB_STATUS["ram_only"]:
            storage_line = (f"\n\n🔴 <b>Storage holati: RAM ONLY</b>\n"
                           f"PostgreSQL ishlamayapti! Xatolar: <b>{DB_STATUS['fail_count']}</b>")
        elif DB_STATUS["last_save_ok"]:
...
async def media_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_any_admin(uid) and state == "add_ep_video":
        ...
        await save_ram_only()
        # ❗ PostgreSQL ga ham — har 3-qismda bir marta + debounce
        if ep_num == 1 or ep_num % 3 == 0:
            # birinchi qism va har 3-qismda — DARHOL PostgreSQL ga
            asyncio.create_task(_do_postgres_save())
        else:
...
def main():
    ...
    # ── Har 5 daqiqada PostgreSQL ga sinxron saqlash ────────
    ...
    async def _startup_notify(context_job):
        try:
            ...
            if ok:
                ...
                storage_msg = f"🟢 Railway PostgreSQL ishlayapti — {now_str}"
            else:
            ...
            await context_job.bot.send_message(
                ADMIN_ID,
                f"🚀 <b>Bot Railway PostgreSQL da ishga tushdi!</b>\n\n"
                f"💾 RAM: <b>{len(RAM.movies)}</b> kino, <b>{len(RAM.users)}</b> user\n"
                f"🐘 Baza: {storage_msg}\n\n"
                f"✅ Barcha ma'lumotlar Railway Postgres da saqlanadi!",
                parse_mode="HTML")
        except Exception as e:
    ...
    logger.info(f"🚀 Bot Railway PostgreSQL da ishga tushdi! RAM: {len(RAM.movies)} kino, {len(RAM.users)} user")
